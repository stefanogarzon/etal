...
3. **AI is advisory and opt-in.** A Groq layer (`llm.py`) *suggests* a topic (possibly a brand-new one) and writes summaries, but never auto-commits — the user confirms every classification. It is **off until the user adds their own Groq key** in Tools (stored in `config.json` under `llm:`, never returned to the frontend). Public builds ship **without** a key. Any failure (offline / rate-limited / no key) falls back to the offline keyword heuristic, so the app always works. **Provider-agnostic:** any OpenAI-compatible endpoint works — the base URL (`llm.base_url`) + model are configurable in Tools (Groq, Google Gemini's OpenAI layer, a local Ollama, OpenRouter, …); summaries use the same model. A failed call raises `LLMError(reason)`; passive paths (ingest) report `ai.error` to the UI and fall back, user actions surface it (HTTP 429 on rate limit). **Batch jobs** (bulk import, `/api/reclassify_ai/*`) are paced to `llm.rpm` (requests/min) and retry on rate-limit, so a slow free-tier run still finishes. `/api/reclassify_ai/start` re-runs AI over saved articles (scope `_uncategorized`/all) → review → apply.
   - **Optional shared key (not for public builds).** `llm.BUILTIN_KEY` is resolved at runtime from `ETAL_GROQ_KEY` → a git-ignored `groq_key.txt` bundled at build → empty. Use it only for a *private* build; never commit `groq_key.txt`, and never ship a key in a public release (it's extractable from the binary and would be abused/auto-revoked).
